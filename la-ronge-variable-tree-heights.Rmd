...
```{r, fig.show='hold', out.width="50%", echo=TRUE, eval=TRUE}
...
ttops$height_95 = ttops$height_95 * 0.95 #avoid re-runs
ttops$height_95_stdev = sd(ttops$height_95)
...
chm_ttops_stdev = stars::st_rasterize(ttops %>% dplyr::select(height_95_stdev, geometry))
...
#chm_ttops_sv = terra::vect(ttops) #only holds one field by default
#chm_ttops_sr = terra::rasterize(chm_ttops_sv, chm, touches=TRUE)
...
Using the simple structural diversity index developed by MacArthur and MacArthur (1961), we derived structural diversity classes according to height heterogeneity. As our target here are height heterogeneity areas (HHA's), we highlight below in red the class showing highest standard deviation. 

Further metrics may consider combining horizontal estimates from within and between sampling units, as well as crown width heterogeneity in areas where there higher concentrations of wolf and veteran trees. Here we simply rely on raster cell boundaries against population deviation as a quick fix. 

```{r, echo=TRUE, eval=TRUE}

hist(chm_ttops_stdev)
...
legend("topright",
       legend = c("low heterogeneity", "slight heterogeneity", "moderate heterogeneity", "HHA's"),
